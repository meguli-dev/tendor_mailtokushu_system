-- 販売形態別マージン率（2026-07-14 ヒアリング反映）
--   バラ売り: 25% / 直送: 15% / 自社配送(ケース): 25%（従来のmargin_rate）
--   値はsettingsテーブルで管理し、将来の管理画面から調整可能にする
insert into youkinavi.settings (key, value, description) values
  ('margin_rate_direct', 0.15, '直送時のマージン率（※要確認: 15%と聞き取り）'),
  ('margin_rate_bara', 0.25, 'バラ売り時のマージン率（※要確認: 25%と聞き取り）')
on conflict (key) do nothing;

-- quote_set をモード別マージンに対応（modeに bara を追加）
...
begin
  ...
  if qty is null or qty <= 0 then
    return jsonb_build_object('error', '数量は1以上を指定してください');
  end if;
  if mode not in ('own', 'direct', 'bara') then
    return jsonb_build_object('error', 'modeは own（自社配送・ケース）/ direct（直送）/ bara（バラ売り）のいずれかを指定してください');
  end if;

  -- 販売形態別マージン
  margin := case mode
    when 'direct' then coalesce(youkinavi.get_setting('margin_rate_direct'), 0.15)
    when 'bara' then coalesce(youkinavi.get_setting('margin_rate_bara'), 0.25)
    else coalesce(youkinavi.get_setting('margin_rate'), 0.25)
  end;
  ...
  steps := steps || jsonb_build_array(
    ...
    format('基準売価 = %s円 × %s（%sマージン%s%%） = %s円/セット',
      cost_sum, 1 + margin,
      case mode when 'direct' then '直送' when 'bara' then 'バラ' else '自社配送' end,
      round(margin * 100), base)
  );
  ...
  if mode = 'direct' then
    ...
    -- own / bara: 送料800円を送料無料ライン頭割りで単価に込み
  ...
  return jsonb_build_object(
    ...
    'cost_price_set', cost_sum,
    'floor_price_set', base,
    'calc_steps', steps,
    'warnings', warnings,
